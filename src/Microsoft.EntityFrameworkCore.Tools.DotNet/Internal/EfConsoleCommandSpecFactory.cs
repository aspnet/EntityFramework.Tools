...
namespace Microsoft.EntityFrameworkCore.Tools.DotNet.Internal
{
    public class EfConsoleCommandSpecFactory
    {
        ...
        public EfConsoleCommandSpecFactory(EfConsoleCommandResolver resolver)
        {
            _resolver = resolver;
        }

        public CommandSpec Create(IProjectContext startupProject, IProjectContext targetProject, bool verbose, IEnumerable<string> additionalArguments)
        {
            if (startupProject.IsClassLibrary)
            {
                throw new OperationErrorException(string.Format(ToolsDotNetStrings.ClassLibrariesNotSupported, startupProject.ProjectName));
            }
