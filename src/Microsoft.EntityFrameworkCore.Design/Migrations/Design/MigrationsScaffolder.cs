    ...
    public class MigrationsScaffolder
    {
        ...
        public virtual MigrationFiles RemoveMigration([NotNull] string projectDir, [NotNull] string rootNamespace, bool force)
        {
                ...
                if (!_modelDiffer.HasDifferences(model, modelSnapshot.Model))
                {
                    ...
                    else
                    {
                        _logger.Value.LogWarning(
                            DesignStrings.NoMigrationFile(migrationFileName, migration.GetType().DisplayName(fullName: false)));
                    }
            ...
            if (model == null)
            {
                ...
                {
                    _logger.Value.LogWarning(
                        DesignStrings.NoSnapshotFile(modelSnapshotFileName, modelSnapshot.GetType().DisplayName(fullName: false)));
                }
            }
            else
            {
        ...
        protected virtual string GetNamespace([CanBeNull] Type siblingType, [NotNull] string defaultNamespace)
        {
            if (siblingType != null)
            {
                ...
                {
                    _logger.Value.LogDebug(DesignStrings.ReusingNamespace(siblingType.DisplayName(fullName: false)));
