...
namespace Microsoft.EntityFrameworkCore.Tools.DotNet.Internal
{
    ...
    {
        private static readonly Assembly _thisAssembly = typeof(Program).GetTypeInfo().Assembly;
        private readonly string _basePath = Path.GetDirectoryName(_thisAssembly.Location);
        private string NetCoreToolDir
            => Path.Combine(_basePath, "..", "..", "tools", "netcoreapp1.0");
        private string DesktopToolDir
